...
if (network !== 'mainnet' && network !== 'mutinynet') throw new Error('Choose RECOVERY_NETWORK=mainnet or mutinynet')
...
if (Boolean(tlsCert) !== Boolean(tlsKey)) throw new Error('Provide both RECOVERY_TLS_CERT and RECOVERY_TLS_KEY')
const server = Bun.serve({
  ...(tlsCert && tlsKey ? { tls: { cert: Bun.file(tlsCert), key: Bun.file(tlsKey) } } : {}),
  port,
  hostname,
  routes: {
    '/': homepage,
