...
export function lightRecoveryHandler(
  ...
  return async (req: Request): Promise<Response> => {
    ...
    const bitcoin =
      /^\/esplora\/(fee-estimates|blocks\/tip\/height|blocks\/tip\/hash|block-height\/[0-9]+|block\/[0-9a-f]{64}(?:\/header|\/status)?|tx(?:\/[0-9a-f]{64}(?:\/hex|\/status|\/outspends|\/outspend\/[0-9]+)?)?|address\/[A-Za-z0-9]{14,100}(?:\/utxo|\/txs(?:\/chain\/[0-9a-f]{64})?)?)$/.exec(
        url.pathname,
      );
    if (
      !bitcoin ||
      (req.method !== "GET" &&
        !(req.method === "POST" && url.pathname === "/esplora/tx"))
    )
    ...
    if (
      ...
      return new Response("Invalid Bitcoin transaction", { status: 400 });
    try {
